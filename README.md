...
## V6.1 Vercel build fix
Vite is configured with `build.target = "esnext"` because MuPDF's browser WASM loader uses top-level await. This fixes the Vercel error `Top-level await is not available in the configured target environment`.
